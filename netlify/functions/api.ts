...
const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    if (mode === "n8n") {
      console.log(`Forwarding payload (${requestType}) to n8n serverless proxy: ${targetWebhook}`);
      let n8nBody: any = {};
      ...
      return {
        ...
        body: JSON.stringify({ success: true, data: responseData, source: "n8n" }),
      };
    } else {
      ...
      // Late initialize/refresh if process.env.GEMINI_API_KEY changed dynamically in container
      const activeAi = ai || (process.env.GEMINI_API_KEY ? new GoogleGenAI({
      ...
      if (requestType === "generate") {
        console.log("Generating direct Instagram content using Gemini AI serverless handler...");
        prompt = `
...
`;
        ...
        console.log(`Editing content using Gemini AI serverless handler... Target: ${editRequest?.editTarget}`);
        prompt = `
...
`;
      ...
      // Supply high quality fallback stock images if missing
      ...
      return {
        statusCode: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        body: JSON.stringify({ success: true, data: parsedData, source: "gemini" }),
      };
    }
  } catch (err: any) {
    console.error("Netlify serverless failure:", err);
    return {
      statusCode: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      body: JSON.stringify({
        success: false,
        error: `Netlify serverless error: ${err.message}`,
      }),
    };
  }
};
